...
public class CacheManager {
	...
	private void initCacheLoader() {
		...
		//datacenter
		//vim
				datacenterCache = CacheBuilder.newBuilder().maximumSize(Constants.MAX_SIZE)
		...
		//cluster
				clusterCache = CacheBuilder.newBuilder().maximumSize(Constants.MAX_SIZE)
	...
	private void loadDC() {
		...
		try {
			ManagedEntity[] entities = new InventoryNavigator(si.getRootFolder()).searchManagedEntities("Datacenter");
			for (ManagedEntity entity : entities){
	...
	public List<DatacenterDTO> getAllDC(){
		...
		if (datacenterCache.asMap().isEmpty())
			loadDC();
		for (Map.Entry<String, DatacenterDTO> entry : datacenterCache.asMap().entrySet()) {
			list.add(entry.getValue());
		}
		return list;
	}
	...
	public List<ClusterDTO> getAllCluster(){
		...
		if (clusterCache.asMap().isEmpty())
			loadCluster();
		for (Map.Entry<String, ClusterDTO> entry : clusterCache.asMap().entrySet()) {
			list.add(entry.getValue());
		}
		return list;
	}
