...
public class Utility {
  ...
  public static HostDTO getHostValues(HostSystem hostSystem) {
    ...
    try {
      ...
      dto.setShutdownSupported(hostSystem.getCapability().shutdownSupported
          + "");
      dto.setBootTime(hostSystem.getRuntime().bootTime.getTime().toString());
      dto.setPowerState(hostSystem.getRuntime().powerState.name());
      dto.setManagementServerIp(hostSystem.getSummary().managementServerIp
          .toString());
  ...
  public static ClusterDTO getClusterValues(ClusterComputeResource cluster)
      throws InvalidProperty, RuntimeFault, RemoteException {
    ClusterDTO dto = new ClusterDTO();
    try {
      dto.setMoid(cluster.getMOR().getVal());
      dto.setType(cluster.getMOR().getType());
      ...
      ManagedEntity[] entities = new InventoryNavigator(cluster)
      ...
      ManagedEntity[] hosts = new InventoryNavigator(cluster)
      ...
      ManagedEntity[] vms = new InventoryNavigator(cluster)
  ...
  public static DatacenterDTO getDatacenterValues(Datacenter datacenter)
      throws InvalidProperty, RuntimeFault, RemoteException {
    ...
    ManagedEntity[] entities = new InventoryNavigator(datacenter)
        .searchManagedEntities("Datastore");
    ArrayList<DatastoreDTO> dsDtos = new ArrayList<DatastoreDTO>();
    for (ManagedEntity entity : entities) {
      Datastore datastore = (Datastore) entity;
    ...
    ManagedEntity[] vms = new InventoryNavigator(datacenter)
        .searchManagedEntities("VirtualMachine");
    ArrayList<VimDTO> vmList = new ArrayList<VimDTO>();
    for (ManagedEntity entity : entities) {
      VirtualMachine vm = (VirtualMachine) entity;
